...
//STORIES TO EXPLORE
app.post('/get-stories-to-explore', async (req, res) => {
    let { id } = req.session,
        _stories = await db.query(
            'SELECT users.id, stories.story_id,stories.user, users.username, users.firstname, users.surname, stories.description, stories.url, stories.type, stories.story_time FROM stories, users, follow_system WHERE follow_system.follow_by = ? AND follow_system.follow_to = stories.user AND stories.user = users.id ORDER BY stories.story_time DESC LIMIT 12',
            [id]
        )
        stories = []

    for (let s of _stories) {
        let isFollowing = await User.isFollowing(id, s.id),
            [{ followers_count }] = await db.query(
                'SELECT COUNT(follow_id) AS followers_count FROM follow_system WHERE follow_to=?',
                [s.id]
            ),
            mutualUsers = await User.mutualUsers(id, s.id)
    ...
    let orderByMutualUsers = _.orderBy(stories, ['mutualUsersCount'], ['desc'])
    res.json(orderByMutualUsers)
})
